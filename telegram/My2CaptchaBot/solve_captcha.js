// Captcha is already sent to other worker.
// Waiting for captcha ...
// Captcha accepted. Waiting for captcha ...
// Captcha #64928441162 closed	
// click the icons in order
// /start
// /stop
// /not_captcha
// /next
...
async function solve() {	
	...
	do {
		imgSrc = getImageSrc();
		console.error("Temp img: " + imgSrc);
		console.error("Last img: " + lastImgSrc);
		console.error(imgSrc != '' && imgSrc != lastImgSrc);
		await sleep(3000);
	} while (!(imgSrc != '' && imgSrc != lastImgSrc));
	lastImgSrc = imgSrc;

	// get text from img
	var text = Tesseract.recognize(imgSrc) 
}
...
async function notCaptcha() {
	await sendText("/not_captcha");
}
...
function getImageSrc() {
	var imgs = document.getElementsByClassName("im_message_photo_thumb");
	if (imgs) {
...
function sendText() {
	var sendBtn = document.getElementsByClassName("btn btn-md im_submit im_submit_send")[0];
	if (sendBtn) {
		sendBtn.click();
	}
}
